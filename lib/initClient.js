...
function createClient(initialState) {
  ...
  const httpLink = new HttpLink({
    ...
    fetch: async (url, options) => {
      ...
      const hrstart = process && process.hrtime ? process.hrtime() : null;
      const result = await fetch(url, options);
      const hrend = process && process.hrtime ? process.hrtime(hrstart) : null;
      console.log(`${url} ${body.operationName || ''}`);
      // console.log(`${body.operationName || ''}`, body.query, body.variables);
      if (hrend) {
